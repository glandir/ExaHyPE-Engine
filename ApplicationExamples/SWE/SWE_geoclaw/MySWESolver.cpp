...
// double SWE::MySWESolver::riemannSolver(double* fL, double *fR, const double*
// qL, const double* qR, const double* gradQL, const double* gradQR, const
// double* cellSize, int direction) {
...
//     flux[0] = 0.5 * (FL[direction][0] + FR[direction][0]) - 0.5 * smax *
//     (qR[0] + qR[3] - qL[0] - qL[3]); flux[1] = 0.5 * (FL[direction][1] +
//     FR[direction][1]) - 0.5 * smax * (qR[1] - qL[1]); flux[2] = 0.5 *
//     (FL[direction][2] + FR[direction][2]) - 0.5 * smax * (qR[2] - qL[2]);
//     flux[3] = 0.5 * (FL[direction][3] + FR[direction][3]);

//     double hRoe = 0.5*(qL[0] + qR[0]);

//     double bm = std::max(qL[3], qR[3]);
//     double Deta = std::max(qR[0]+qR[3] - bm, 0.0) - std::max(qL[0]+qL[3] -
//     bm, 0.0);
