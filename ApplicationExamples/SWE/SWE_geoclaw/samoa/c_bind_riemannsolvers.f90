module c_bind_riemannsolvers
  ...
  subroutine c_bind_geoclaw_solver_dp(i_solver, i_maxIter, i_numberOfFWaves, i_hL, i_hR, i_huL, i_huR, i_hvL, i_hvR, i_bL, i_bR, &
      i_dryTol, i_g, o_netUpdatesLeft, o_netUpdatesRight, o_maxWaveSpeed ) bind(c)

  use, intrinsic :: iso_c_binding
  !variable declaration
    !input
      integer (kind=c_int), intent(in)                     :: i_solver, i_maxIter, i_numberOfFWaves

      real (kind=c_double), intent(inout)                  :: i_hL,i_hR, i_huL,i_huR, i_hvL,i_hvR,i_bL,i_bR;
      real (kind=c_double), intent(in)                     :: i_dryTol,i_g

    !output
      real (kind=c_double), dimension(3), intent(out)      :: o_netUpdatesLeft, o_netUpdatesRight;
      real (kind=c_double), intent(out)                    :: o_maxWaveSpeed;

    !local
      integer (kind=c_int)                                 :: waveNumber, equationNumber;
      real (kind=c_double), dimension(i_numberOfFWaves)    :: waveSpeeds;
      real (kind=c_double), dimension(3, i_numberOfFWaves) :: fWaves;
      real (kind=c_double), dimension(3)                   :: wall;
      real (kind=c_double)                                 :: hstar, hstartest, uL, uR, vL, vR, uhat, chat, delphi;
      real (kind=c_double)                                 :: sL, sR, sRoe1, sRoe2, sE1, sE2, s1m, s2m;
  ...
  wall(2) = 1.d0
  wall(3) = 1.d0
  if (i_solver == GEOCLAW_AUG_RIEMANN) then
