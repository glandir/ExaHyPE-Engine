...
/*
 * Simulates a channel with a linearly rising ramp at the left end of the
 * channel. To be used with WALL boundaries.
 */
...
void SWE::RunUpLinear(const double* const x, double* Q) {
  ...
  vars.hu() = -vars.h() * as *
              (1.0 / pow(cosh(sqrt(3 * as / 4) * (x[0] - xs)), 2)) *
              sqrt(grav / d);
  vars.hv() = 0.0;
}
 ...
 * the channel. To be used with WALL boundaries.
 */
void SWE::RunUpLinearY(const double* const x, double* Q) {
  MySWESolver::Variables vars(Q);
  ...
  vars.b() = (x[1] > xr) ? 0 : (-d / xr) * x[1] + d;
  vars.h() = (x[1] > xr) ? d : (d / xr) * x[1];
  vars.h() += as * (1.0 / pow(cosh(sqrt(3.0 * as / 4.0) * (x[1] - xs)), 2));
  vars.hu() = 0.0;
  vars.hv() = -vars.h() * as *
              (1.0 / pow(cosh(sqrt(3 * as / 4) * (x[1] - xs)), 2)) *
              sqrt(grav / d);
}
...
void SWE::RunUpShelf(const double* const x, double* Q) {
  ...
  vars.hu() +=
      -vars.h() * aw * (1.0 / pow(cosh(x[0] - xw), 2)) * sqrt(grav / d2);
...
void SWE::OscillatingLake(const double* const x, double* Q) {
  ...
  vars.h() =
      max(0.0, 0.05 * (2 * x[0] * cos(omega * 0) + 2 * x[1] * sin(omega * 0)) +
                   0.075 - vars.b());
...
// width = 70.0,10.0
// offset = -10.0,0.0
void SWE::SolitaryWaveOnSimpleBeach(const double* const x, double* Q, double Hd,
                                    double d) {
  MySWESolver::Variables vars(Q);
  ...
  double eta = H * (1 / (std::cosh(gamma * (x[0] - (x0 + L)) / d))) *
               (1 / (std::cosh(gamma * (x[0] - (x0 + L)) / d)));
  ...
  if (x[0] < 0) {
    ...
    vars.h() = H * (1 / (std::cosh(gamma * (x[0] - (x0 + L)) / d))) *
                   (1 / (std::cosh(gamma * (x[0] - (x0 + L)) / d))) +
               d;
    vars.b() = 0;
  }
